...
namespace Discord_Bot.Modulos
{
    public class AnilistSlashCommands : SlashCommandModule
    {
        ...
        public async Task Anime(InteractionContext ctx, [Option("Anime", "Nombre del anime a buscar")] string anime)
        {
            ...
            try
            {
                var data = await funcionesAnilist.GetAnilistMedia(context, anime, "anime");
                if (data != null && data.Data != null)
                {
                    ...
                    var elegido = await funciones.GetElegido(context, opc);
                    if (elegido > 0)
                    {
        ...
        public async Task Manga(InteractionContext ctx, [Option("Manga", "Nombre del manga a buscar")] string manga)
        {
            ...
            try
            {
                var data = await funcionesAnilist.GetAnilistMedia(context, manga, "manga");
                if (data != null && data.Data != null)
                {
                    int cont = 0;
                    List<string> opc = new();
